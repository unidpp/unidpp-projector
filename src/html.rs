//! The consumer HTML serialization of the render (TODO.impl 224): the
//! same presentation, a second wire format. `/render` negotiated to
//! `text/html` serves a standalone page built **from the render JSON
//! document** — sections, localized labels, formatted values, stated
//! gaps, coverage — so there is exactly one presentation computation
//! and the HTML cannot disagree with the JSON (MobileQR dissection
//! lesson: one renderer, presentation as data — never per-template
//! forks).
//!
...
pub fn accept_prefers_html(accept: Option<&str>) -> bool {
    ...
    let first = accept
        ...
        .trim()
        .to_ascii_lowercase();
    first == "text/html" || first == "application/xhtml+xml"
}
