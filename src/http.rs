...
//! the house pattern (ported from `unidpp-issuer`'s `http`, itself
//! ported from `unidpp-registry`'s test support and
//! `unidpp-resolver`'s `httpc`): no reqwest/hyper-client/TLS
//! dependency in either the service or its tests. `http://` only; the
//! projector talks to sibling services on loopback or inside the
//! same trust boundary, and production TLS terminates at a fronting
//! proxy.
//!
//! Shared by two consumers:
//! - [`crate::registry::RegistryClient`] (reading profile and unit
//!   items from a `unidpp-registry` instance), and
...
/// Default read timeout (loopback sibling service; a slow registry
...
impl Url {
    ...
    /// Percent-encode a path segment (same escaping as query values:
    /// `/`, `?`, `&`, `=`, `:` etc. must not be read structurally).
...
/// Convenience wrapper for JSON requests (the registry client and the
...
mod tests {
    ...
    fn path_segment_encoding() {
        // URN-shaped item ids keep their colons structural-free.
        assert_eq!(
            Url::encode_path_segment("urn:unidpp:profile:eu-espr-electronics"),
            "urn%3Aunidpp%3Aprofile%3Aeu-espr-electronics"
        );
        assert_eq!(Url::encode_path_segment("a/b c"), "a%2Fb%20c");
    }
